import { ColorTheGridPuzzle, GridCardContent } from './types.js';
...
export class ColorGridEngine {
  ...
  private static generateCandidate(level: number, attempt: number): ColorTheGridPuzzle {
    // 1. Select Rule Tier based on Level
    // Level 1-2: Tier 1 (Atomic)
    // Level 3-5: Tier 2 (Conjunction / Disjunction)
    // Level 6-8: Tier 3 (Relational / Positional / Negation)
    // Level 9+: Tier 4 (Multi-class / Complex Boundary)
    const tier = level <= 2 ? 1 : (level <= 5 ? 2 : (level <= 8 ? 3 : 4));

    const ruleDefs: ColorRuleDef[] = [
      // Tier 1: Atomic predicates
      {
        tier: 1,
      ...
      // Tier 2: Conjunction (AND) / Disjunction (OR)
      {
        tier: 2,
      ...
      // Tier 3: Positional / Relational & Negation
      {
        tier: 3,
    ...
    // Filter to rules matching current tier
    ...
    // 2. Generate 4 grid cards with adversarial boundary testing
    const chars = 'ABEZXYK';
